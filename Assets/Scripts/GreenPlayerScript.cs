...
public class GreenPlayerScript : MonoBehaviour
{
    ...
    {
        if (GameManagerScript.mGameManager.mRolledDice.CompareTag("Green") && mFlag == 0 && GameManagerScript.mGameManager.mNumberGot == 6)
        {
            MoveToken();
        }
        else if (mFlag == 1 && mTokenOut == true && GameManagerScript.mGameManager.mRolledDice.CompareTag("Green") && GameManagerScript.mGameManager.mNumberGot <= mNumberOfStepsRemaining)
        {
            MoveToken();
        }
    ...
    {
        if (mGreenCanMove && GameManagerScript.mGameManager.mDiceRolled == true)
        {
    ...
    {
        if (GameManagerScript.mGameManager.mLastToken == true && mAnotherChance == false && mTokenMoved == false)
        {
    ...
    private void OnTriggerStay2D(Collider2D collision)
    {
        ...
        if (mCircleCollider.bounds.Intersects(mTriggerCollider.bounds) && GameManagerScript.mGameManager.mRolledDice.CompareTag("Green"))
        {
            FindInitialPositionRed();
            FindInitialPositionBlue();         
            FindInitialPositionYellow();
            if (mGreenTokenMoving == false && mPath[mTokenPosition].transform.position == collision.gameObject.transform.position)
            {
    ...
    {
        if (GameManagerScript.mGameManager.mNumberGot == 6 && GameManagerScript.mGameManager.mRolledDice.CompareTag("Green") && GameManagerScript.mGameManager.mDiceRolled == true)
        {
            mGreenToken.CircleEnable();
        }
        else if (GameManagerScript.mGameManager.mNumberGot < 6 && GameManagerScript.mGameManager.mRolledDice.CompareTag("Green") && GameManagerScript.mGameManager.mDiceRolled == true)
        {
    ...
    {
        if (GameManagerScript.mGameManager.mGreenPlayerFinished == 4)
        {
            mGreenToken.StopMove();
            GameManagerScript.mGameManager.mPlayerFinished.Add("Player 4");
            mGreenToken.mNumberRenderer.sprite = GameManagerScript.mGameManager.mNumbers[GameManagerScript.mGameManager.mNumberIndex];
            mGreenToken.mNumberRenderer.transform.position = new Vector3(mGreenToken.mTokenCentreX, mGreenToken.mTokenCentreY, -7);
            mGreenToken.mNumberRenderer.transform.localScale = new Vector2(10f, 10f);
